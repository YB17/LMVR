...
from llava.model_lora_vl import *
...
class ModelArguments:
    ...
    #tune_mm_mlp_adapter_alignment_head: bool = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_bias: str = "none"
    use_lora_v2: str = None
    pretrain_ckpt_path: str = None
    full_finetune: bool = False
    lora_stem_no_grad: str = None
    enable_vl: str = 'mlp_attn'
...
def find_all_linear_names(model):
    cls = torch.nn.Linear
    lora_module_names = set()
    multimodal_keywords = ['mm_projector', 'vision_tower', 'lm_head', 'proj_head']
    for name, module in model.named_modules():
...
def train():
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(
                ...
                use_lora_v2=training_args.use_lora_v2,
                lora_stem_no_grad=training_args.lora_stem_no_grad,
                enable_vl=training_args.enable_vl,
                align_enable=training_args.align_enable,
                alignment_weight=training_args.alignment_weight,
                feature_size=training_args.feature_size,
                **bnb_model_from_pretrained_args
            )
    else:
    ...
    #training_para_list = []
    #if training_args.freeze_lora:
        # freeze non-lora parameters
    #    for n, p in model.named_parameters():
    #        if "lora_" in n:
    #            p.requires_grad = False
    #        else:
    #            training_para_list.append(n)
    #if not training_args.full_finetune: # default: only finetune lora parameters
        # freeze non-lora parameters
    #    for n, p in model.named_parameters():
    #        if "lora_" not in n:
    #            p.requires_grad = False
    ...
    #    print("Training parameters:", training_para_list)
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    if training_args.pretrain_ckpt_path:
         msg = load_sharded_checkpoint(model.base_model.model, training_args.pretrain_ckpt_path, strict=False)
